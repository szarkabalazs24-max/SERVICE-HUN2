...
class GiveawayModal(ui.Modal, title='Nyereményjáték Beállítása'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await self.process_winners(interaction, self.prize.value, self.winner_count.value, msg.id)

    async def process_winners(self, interaction, prize, count, msg_id):
        conn = sqlite3.connect('giveaway.db')
        c = conn.cursor()
